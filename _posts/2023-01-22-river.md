...
![featured_image](jan2023_featured.webp)
...
![notification](notification-1.webp)
...
![sandwich_rgb](sandwich_rgb_noaa.webp)

1. Use **wget** to download the images we want. Lets grab all of the images that contain “1000×1000” in their name (1000×1000 indicates the pixel size of the image).
...
2. Now that the images are downloaded use **ffmpeg** to convert them into a video.

```bash
ffmpeg -framerate 10 -pattern_type glob -i '*.jpg' -c:v libx264 -r 30 -pix_fmt yuv420p output.mp4
```

[![video](video.png)](https://www.youtube.com/watch?v=kJKkdyobQA8)